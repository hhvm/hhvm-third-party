...
#ifdef __APPLE__
#include "thrift/lib/cpp/util/pthread-timed-lock-shim.h"
#endif
...
static void init_with_kind(pthread_mutex_t* mutex, int kind) {
  ...
  if (pthread_mutexattr_init(&mutexattr) != 0) {
    throw SystemResourceException("pthread_mutexattr_init failed");
  }

  if (pthread_mutexattr_settype(&mutexattr, kind) != 0) {
    throw SystemResourceException("pthread_mutexattr_settype failed");
  }

  if (pthread_mutex_init(mutex, &mutexattr) != 0) {
    throw SystemResourceException("pthread_mutex_init failed");
  }

  if (pthread_mutexattr_destroy(&mutexattr) != 0) {
    throw SystemResourceException("pthread_mutexattr_destroy failed");
  }
}
...
class Mutex::impl {
 public:
  ...
  impl(int type) : initialized_(false) {
#ifndef THRIFT_NO_CONTENTION_PROFILING
    profileTime_ = 0;
#endif
    init_with_kind(&pthread_mutex_, type);
    initialized_ = true;
  }
  ...
  bool timedlock(int64_t milliseconds) const {
    ...
    struct timespec ts;
    Util::toTimespec(ts, Util::currentTime() + milliseconds);
...
Mutex::Mutex(int type) : impl_(new Mutex::impl(type)) {}
...
bool Mutex::isLocked() const { return impl_->isLocked(); }

void Mutex::DEFAULT_INITIALIZER(void* arg) {
  pthread_mutex_t* pthread_mutex = (pthread_mutex_t*)arg;
  int ret = pthread_mutex_init(pthread_mutex, nullptr);
  DCHECK(ret == 0);
}
...
public:
  ...
  bool timedRead(int64_t milliseconds) const {
    struct timespec ts;
    Util::toTimespec(ts, Util::currentTime() + milliseconds);
    PROFILE_MUTEX_START_LOCK();
    int ret = pthread_rwlock_timedrdlock(&rw_lock_, &ts);
    PROFILE_MUTEX_NOT_LOCKED();  // not exclusive, so use not-locked path
    return ret == 0;
  }

  bool timedWrite(int64_t milliseconds) const {
    struct timespec ts;
    Util::toTimespec(ts, Util::currentTime() + milliseconds);
    PROFILE_MUTEX_START_LOCK();
    int ret = pthread_rwlock_timedwrlock(&rw_lock_, &ts);
    if (ret == 0) {
      PROFILE_MUTEX_LOCKED();
      return true;
    }
...
bool ReadWriteMutex::timedRead(int64_t milliseconds) const {
  return impl_->timedRead(milliseconds);
}

bool ReadWriteMutex::timedWrite(int64_t milliseconds) const {
  return impl_->timedWrite(milliseconds);
}
